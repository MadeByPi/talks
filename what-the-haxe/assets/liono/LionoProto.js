...
ApplicationMain.embed = $hx_exports.openfl.embed = function(elementName,width,height,background) {
	...
	ApplicationMain.urlLoaders.set("assets/example.json",urlLoader);
	...
	if(ApplicationMain.total == 0) ApplicationMain.start(); else {
		...
		while( $it0.hasNext() ) {
			...
			urlLoader1.load(new openfl.net.URLRequest(path));
		}
		var _g = 0;
...
Main.prototype = $extend(openfl.display.Sprite.prototype,{
	...
	,init: function() {
		...
		this.inited = true;
		var json = openfl.Assets.getText("assets/example.json");
		if(json.indexOf("\\") != -1) json = new EReg("\\","g").replace(json,"");
		openfl.Lib.current.addChild(new LineDisplay(this.stage,JSON.parse(json)));
	}
	,added: function(e) {
...
var LineDisplay = function(stage,data) {
	openfl.display.Sprite.call(this);
	var sw = stage.stageWidth * 3;
	var sh = stage.stageHeight;
	var sh4 = sh / 6;
	var maxIndent = data.info.maxIndent;
	var maxLength = data.info.maxLength;
	var maxValue = data.info.maxValue;
	var x = .0;
	var y = .0;
	var w = sw / data.lines.length;
	var h = .0;
	var _g = 0;
	var _g1 = data.lines;
	while(_g < _g1.length) {
		var line = _g1[_g];
		++_g;
		var i = line.indent / maxIndent;
		var l = line.length / maxLength;
		var v = line.value / maxValue;
		y = sh - i * sh4 - sh4 * 3;
		h = 16 + 64 * v + 64 * l;
		this.get_graphics().beginFill(16777215 * l * v | 0,.5 + .5 * l);
		this.get_graphics().drawRect(x,y,w,h);
		x += w;
	}
	this.cacheAsBitmap = true;
	motion.Actuate.tween(this,12,{ x : -(sw - stage.stageWidth)}).reflect().repeat().ease(new motion.easing.QuadEaseInOut());
};
$hxClasses["LineDisplay"] = LineDisplay;
LineDisplay.__name__ = ["LineDisplay"];
LineDisplay.__super__ = openfl.display.Sprite;
LineDisplay.prototype = $extend(openfl.display.Sprite.prototype,{
	__class__: LineDisplay
});
var List = function() {
	this.length = 0;
};
